<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <link rel="stylesheet" href="../resources/css/libraryInfo.css">
    <link rel="stylesheet" href="../resources/css/container.css">
    
<div id="container">
	<jsp:include page="/WEB-INF/views/common/header.jsp"></jsp:include>
	
	<div id="subtitleArea">
            <div id="subtitle">도서관 소개</div>
            <p>메인</p> <p>></p> <p>도서관 소개</p>
        </div>
        <div class="section-content">
            <p>
            <b>제이 도서관에 오신 것을 환영합니다.</b><br>
			<br>
			새롭게 문을 연 제이 도서관은 지식과 영감을 찾아 떠나는 여러분의 첫걸음과 함께하고자 합니다.<br>
			신생 도서관임에도 불구하고, 99만권에 달할뻔 했지만 500권인 방대한 서적을 갖춤으로 여러분들의 지적 호기심을 충족시켜 드릴 준비를 마쳤습니다. <br>
			<br>
			최신 베스트셀러부터 오랜 시간 사랑 받아온 고전, 전문 분야 서적에 이르기까지 다채로운 분야의 도서들을 한 곳에서 만나보세요. <br>
			자신만의 보물을 발견할 수 있는 공간이 될 것입니다.<br>
			<br>
			또한, 웹 사이트를 통해 방대한 서적을 한번에 찾아볼 수 있는 도서 검색 기능, 찾은 서적의 대출 유무와 대출/반납 정보 등의 확인, <br>
			추천 도서 목록 등 다양한 서비스를 편리하게 이용하실 수 있습니다.<br>
			<br>
			제이 도서관은 여러분의 삶에 책 읽는 즐거움을 더하기 위해 언제나 최선을 다하겠습니다.<br>
			많은 관심과 방문 부탁드립니다.
            </p>
            <br>
            <br>
            <br>
            <br>
            <br>
            <p>문의전화) 02 - 130 - 4983 / 독서관리팀</p>
        </div>


        <div id="subtitleArea">
            <div id="subtitle">찾아오시는 길</div>
        </div>
       
        <div id="map-container">
            <div id="map-overlay"></div>
            <div id="map-marker"></div>
            <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3162.247475306853!2d126.983005!3d37.568470!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x357ca275c88c7f07%3A0x6d9a4f47514b840e!2z7ISc7Jq47Iq57IKY66Gc66mU7JiB7Ja4IOyEnOq1rCDrgqjqta3roZwgMTIw!5e0!3m2!1sko!2skr!4v1624444444444!5m2!1sko!2skr&zoom=15&gestureHandling=none" width="100%" height="100%" style="border:0;" allowfullscreen="" loading="lazy" scrolling="no"></iframe>
        </div>
	
	
	<jsp:include page="/WEB-INF/views/common/footer.jsp"></jsp:include>
</div>